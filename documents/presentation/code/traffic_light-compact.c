    ...
    switch (myLight->state) {
        case ST_GREEN: {
            if (myLight->timer == 0) {
                ...
                myLight->state = ST_ORANGE; } break; }
        case ST_ORANGE: {
            if (myLight->timer == 0) {
                myLight->timer = TIME_RED;
                myLight->state = ST_RED; } break; }
        case ST_RED: {
            if (myLight->timer == 0) {
                myLight->timer = TIME_GREEN;
                myLight->state = ST_GREEN; } break; } }
    if (myLight->timer) { myLight->timer--; } }

void LightTest(void)
{ //todo: expand for orange and red states
    myLight->state = ST_GREEN;
    myLight->timer = 1;
    LightExecute();
    TEST_ASSERT_EQUAL_INT(ST_GREEN,myLight->state);
    TEST_ASSERT_EQUAL_INT(1,myLight->timer);

    myLight->state = ST_GREEN;
    myLight->timer = 0;
    LightExecute();
    TEST_ASSERT_EQUAL_INT(ST_ORANGE,myLight->state);
